...
Embedding-based retriever: `retrieveContext(query)` embeds the query with `gemini-embedding-001` (768-dim, via `src/lib/embeddings.ts`) and scores it against a prebuilt vector store at `src/lib/guidelines/vector-store.json` (generated by the root `index-guides.ts` script, run with `npm run index:guides`). All retrieval failures degrade to `""` so a missing or broken store never blocks a verdict.
